  ...
  try {
    ...
    switch (payload.action) {
      ...
      case "add_master":
        ...
        break;
      case "edit_master":
        result = handleEditMaster(payload);
        break;
      case "delete_master":
...
function doGet(e) {
  return ContentService.createTextOutput(
    JSON.stringify({ status: "success", message: "API Aktif - Phase 3" }),
  ).setMimeType(ContentService.MimeType.JSON);
}
...
function handleGetMaster() {
  ...
  const bgn = getSheetData("Master_Bangunan").map((r) => ({
    id: r[0],
    nama: r[1],
  }));
  const rng = getSheetData("Master_Ruang").map((r) => ({
    id: r[0],
    id_bangunan: r[1],
    nama: r[2],
  }));
...
function handleEditMaster(payload) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === id) {
      if (type === "ruang")
        sheet.getRange(i + 1, 3).setValue(nama); // kolom 3
...
function handleDeleteMaster(payload) {
  ...
  if (bkSheet) {
    ...
    for (let j = 1; j < bkData.length; j++) {
      if (type === "bangunan" && bkData[j][9] === id)
        throw new Error("Bangunan ini sedang digunakan di riwayat transaksi.");
      if (type === "ruang" && bkData[j][10] === id)
        throw new Error("Ruang ini sedang digunakan di riwayat transaksi.");
      if (type === "pos" && bkData[j][3] === id)
        throw new Error(
          "Pos Belanja ini sedang digunakan di riwayat transaksi.",
        );
    }
  }
...
function handleGetBukuKas(payload) {
  ...
  const mapBgn = {};
  mstData.bangunan.forEach((b) => (mapBgn[b.id] = b.nama));
  const mapRng = {};
  mstData.ruang.forEach((r) => (mapRng[r.id] = r.nama));
  const mapPos = {};
  mstData.pos.forEach((p) => (mapPos[p.id] = p.nama));
...
function handleExportLaporan(payload) {
  ...
  const filteredData = bkData.filter(
    (d) => id_ruang === "all" || d.id_ruang === id_ruang,
  );
  ...
  if (outputData.length > 0) {
    ...
    // Formatting border
    newSheet
      .getRange(6, 1, outputData.length, 7)
  ...
  return {
    ...
    data: {
      ...
      filename: `Laporan_BukuKas_${namaRuang}.${exportFormat}`,
    },
  };
}
...
function setupDatabase() {
  ...
  const sheetsConfig = [
    {
      ...
      dummyData: [
        ["U-001", "admin", "12345", "kepsek", "Kepala Sekolah", true],
        ["U-002", "bendahara", "12345", "bendahara", "Bendahara Sekolah", true],
      ],
    },
    {
  ...
  sheetsConfig.forEach((config) => {
    ...
    if (config.dummyData.length > 0)
      sheet
        .getRange(2, 1, config.dummyData.length, config.headers.length)
        .setValues(config.dummyData);
  ...
  tempSheet
    .getRange("A1:G1")
    .merge()
    .setValue("LAPORAN BUKU KAS REVITALISASI")
    .setFontWeight("bold")
